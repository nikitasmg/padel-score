...
**Tech Stack:** Next.js 16 (App Router, Turbopack) + React 19 + TypeScript, Tailwind CSS v4 (CSS-first `@theme`), shadcn/ui v4 (base-nova), Zustand (+ persist), Serwist (@serwist/next), Vitest, next/font (Manrope + JetBrains Mono).
...
- **Tailwind v4**: токены и keyframes объявляются в `@theme` внутри `app/globals.css`, НЕ в `tailwind.config.ts` (его нет). PostCSS — `@tailwindcss/postcss`. Классы (`bg-accent`, `text-ink`, `font-display`, `animate-ring` и т.п.) используются как обычно.
- **Дисплейный шрифт — Manrope** (через `next/font/google`, subsets `["latin","cyrillic"]`), CSS-переменная `--font-manrope`, маппится в `font-display`. Моно — **JetBrains Mono** (`--font-mono`). Archivo НЕ используется (нет кириллицы).
- **PWA — Serwist** (`@serwist/next` + `app/sw.ts`), не `@ducanh2912/next-pwa`.
...
npm i -D typescript @types/react @types/node @types/react-dom tailwindcss@latest @tailwindcss/postcss postcss vitest @vitejs/plugin-react jsdom @testing-library/react @testing-library/jest-dom
```
...
Create `next.config.mjs` (PWA/Serwist подключается в Task 16, здесь — чистый конфиг):
```js
...
Create `postcss.config.mjs` (Tailwind v4 PostCSS-плагин):
```js
export default { plugins: { "@tailwindcss/postcss": {} } };
```

Tailwind v4 — **без `tailwind.config.ts`**. Все токены/keyframes объявляются в `app/globals.css`
через `@theme` (см. Step 3). Если `tailwind.config.ts` остался от прежней попытки — удалить.
...
Create `app/globals.css` (Tailwind v4: импорт + наши токены/keyframes в `@theme`; shadcn init
добавит сюда свои CSS-переменные ниже — не удалять их):
```css
@import "tailwindcss";

@theme {
  --color-bg: #0a0b0a;
  --color-surface: #101210;
  --color-surface2: #121412;
  --color-surface3: #0c0d0c;
  --color-accent: #c6f24e;
  --color-ink: #f4f5f1;
  --color-ink2: #eef0ea;
  --color-ink3: #cdd1c9;
  ...
  --color-muted4: #3a3f38;
  --color-live: #ff5c38;
  --color-broadcast: #070807;

  --font-display: var(--font-manrope), system-ui, sans-serif;
  --font-mono: var(--font-jetbrains), monospace;

  --animate-pulse2: pulse2 1.8s infinite;
  --animate-ring: ring 2s infinite;
}

@keyframes pulse2 { 0%,100% { opacity: 1 } 50% { opacity: .3 } }
@keyframes ring { 0% { box-shadow: 0 0 0 0 rgba(198,242,78,.45) } 100% { box-shadow: 0 0 0 12px rgba(198,242,78,0) } }

html, body { margin: 0; background: #0a0b0a; -webkit-font-smoothing: antialiased; }
* { box-sizing: border-box; }
.tnum { font-variant-numeric: tabular-nums; }
```
> В Tailwind v4 цвета из `--color-*` дают классы `bg-accent`, `text-ink`, … а `--animate-*` — `animate-ring`.

Create `app/layout.tsx` (шрифты Manrope + JetBrains Mono):
```tsx
...
```
> Manrope max weight = 800 (нет 900). Заголовки дизайна с `font-black`/900 берут 800 — приемлемо.
...
### Task 16: PWA — манифест, Serwist service worker, иконки

**Files:**
- Create: `app/manifest.ts`, `app/sw.ts`, `public/icon-192.png`, `public/icon-512.png`
- Modify: `next.config.mjs` (обернуть Serwist), `.gitignore` (добавить `public/sw*.js`)

**Interfaces:**
- Produces: установимый манифест; service worker через **Serwist** (`@serwist/next`), совместимый с Turbopack/Next 16.

- [ ] **Step 0: Установить и подключить Serwist**
...
Добавить в `.gitignore`: строки `public/sw.js` и `public/swe-worker-*.js`.
...
Run: `npm run build && npm run start` → открыть в браузере, проверить в DevTools → Application: манифест валиден, Serwist service worker (`/sw.js`) зарегистрирован, приложение установимо.
Expected: манифест и SW присутствуют; ошибок сборки нет. (SW отключён в dev — проверять на prod-сборке.)

- [ ] **Step 4: Commit**

```bash
git add app/manifest.ts app/sw.ts next.config.mjs .gitignore public/icon-192.png public/icon-512.png
git commit -m "Добавить PWA: манифест, Serwist service worker и иконки"
```
